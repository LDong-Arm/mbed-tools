...
def read_device_files(directory_paths: Iterable[pathlib.Path]) -> DeviceFileInfo:
    """Read data from files contained on an mbed enabled device's USB MSD.

    If details.txt exists and it contains a product code, then we will use that code.
    If not then we try to grep the code from the mbed.htm file.
    We extract an OnlineID from mbed.htm as we also make use of that information to find a board entry in Mbed OS's
    various target databases and JSON files.
    ...
    if not device_file_paths:
        ...
        logger.warning(
            f"No files were found in the device's MSD. The following paths were searched:\n{paths}."
            "\nThis device may not be identifiable as Mbed enabled. Check the files exist, are not "
            "hidden and are not corrupted."
        )
...
def _read_details_txt(file_contents: str) -> dict:
    """Parse the contents of the daplink compatible devices' details.txt.
...
def _get_device_file_paths(directories: Iterable[pathlib.Path]) -> List[pathlib.Path]:
    return [path for directory in directories for path in directory.iterdir() if _is_valid_file(path)]
...
def _is_valid_file(file: pathlib.Path) -> bool:
    """Checks whether the file is not hidden."""
    return not file.name.startswith(".")


def _is_mbed_htm(path: pathlib.Path) -> bool:
